...
The usage of the built-in wire protocol is now removed. You will have to install
`cucumber-wire` and add `require 'cucumber/wire'` in your support code in order
to continue using it, as explained in [The differences with cucumber 7.1.0](#the-differences-with-cucumber-710).

# The differences with cucumber 7.1.0
...
When using cucumber 7.1.0 - the wire protocol will work as before, but you will notice
...
And add `require 'cucumber/wire'` in your support code. If you do not have any support
code yet, create a support file - for example `features/support/wire.rb`.
...
The wire protocol will still be available by explicitely using the `cucumber-wire` gem.
